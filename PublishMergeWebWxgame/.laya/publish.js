...
gulp.task("compressJs", ["compressJson"], function () {
	// 替换js/VersionConfig的版本号
	publishversion.modifyVersionConfigJSForReleaseWeb();
...
// 清理wxgame
gulp.task("copyJsToWxgameClear", ["compressImage"], function (cb) 
{
	// 清理目录
	if (config.copyJsToWxgame && config.copyJsToWxgameIsClear) 
	{
		var releaseDirWX = path.join(workSpaceDir, "release", "wxgame").replace(/\\/g, "/");
		del(releaseDirWX, { force: true }).then(paths => {
			cb();
		});
	} else cb();
});
...
// 拷贝js文件到wxgame
gulp.task("copyJsToWxgame", ["copyJsToWxgameClear"], function () {
	if (config.copyJsToWxgame) 
	{
		var releaseDirWX = path.join(workSpaceDir, "release", "wxgame").replace(/\\/g, "/");
		// 拷贝js文件
		return gulp.src(config.compressJsFilter, { base: releaseDir })
			.pipe(gulp.dest(releaseDirWX));
	}
});


// index.js 文件加上版本注释
gulp.task("versionIndex", ["copyJsToWxgame"], function (cb) {
	if (config.versionIndex) 
	{
		var d = new Date();
		var dstr = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + "  " + d.getTime()

		var indexPath = releaseDir + "/index.js";
		var text = `//date: ${dstr} \n` ;
		var code = fs.readFileSync(indexPath,  { encoding: 'utf8', flag: 'r' });
		text += code;
		fs.writeFileSync(indexPath, text, {flag: 'w', encoding:'utf8'});
		cb();
	} else cb();
});

// 生成版本管理信息
gulp.task("version1", ["versionIndex"], function () {
	if (config.version) {
...
// version.json 加入版本号
gulp.task("versionAddVer", ["version2"], function (cb) {
	if (config.versionAddVer) 
	{
		var ver = publishversion.getResVersion(false);
		let versionPath = releaseDir + "/version.json";
		let versionCon = fs.readFileSync(versionPath, "utf8");
		versionCon = JSON.parse(versionCon);
		versionCon.__ResVersion__ = ver;
		var text =  JSON.stringify(versionCon, null, 4);
		fs.writeFileSync(versionPath, text, {flag: 'w', encoding:'utf8'});
		cb();
	} else cb();
});

// 筛选4M包
gulp.task("packfile", ["versionAddVer"], function() {
	if (config.version) {
